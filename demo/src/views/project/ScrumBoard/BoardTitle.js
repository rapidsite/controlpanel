...
const RenameForm = ({ title, closeRenameForm, columns, ordered }) => {
    ...
    const onFormSubmit = async (newTitle) => {
        ...
        let obj = headerList?.find(o => o.title === title);

        const index = headerList.indexOf(obj);

        const x = headerList.splice(index, 1);

        const temp = new Object();
        temp.title = newTitle
        temp.listId = newTitle
        
        headerList.push(temp)

        const { error } = await supabase.from('admin_core').update({headers: headerList}).eq('id', 1)
                ...
                <div className="px-6 pb-6 pt-2 flex">
                    ...
                    <div className="ml-4 rtl:mr-4">
                        ...
                        <p>
                            ...
                            tickets under this board will be moved to the 'Other' group.
                        </p>
                    </div>
                </div>
